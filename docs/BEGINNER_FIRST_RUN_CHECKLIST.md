...
The Studio page shows a **dismissible “Getting started” bar** (first visit per browser) with the same high-level steps, **Copy** buttons for the preflight and launch commands (clipboard API with a simple fallback), a link to **`/api/v1/health`**, and a **More help** action that opens a compact in-browser summary (localized). The full checklist file remains in the repo as `docs/BEGINNER_FIRST_RUN_CHECKLIST.md`.
